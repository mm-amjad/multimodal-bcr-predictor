...
def run_cross_validation(random_seed, 
    ...
    Args:
        ...
        patient_dataset (TensorDataset): Complete tensor dataset with histology, radiology and time to BCR ground truth lavbels
    ...
    for fold, (train_ids, test_ids) in enumerate(skf.split(labels_tensor[:, 1], labels_tensor[:, 1])):
        ...
        all_rad = patient_dataset.tensors[1]  # Full rad embedding tensor
        all_labels = patient_dataset.tensors[2] # Full labels tensor

        # Extract train/test sets
        hist_train = all_hist[train_ids]
        hist_test = all_hist[test_ids]
        rad_train = all_rad[train_ids]
        rad_test = all_rad[test_ids]
        labels_train = all_labels[train_ids]
        labels_test = all_labels[test_ids]

        # Split train set into 80/20 train/val
        hist_train, hist_val, rad_train, rad_val, labels_train, labels_val = train_test_split(
            hist_train, rad_train, labels_train, test_size=0.2)
        ...
        rad_train_2d = rad_train.view(rad_train.size(0), -1).numpy()
        rad_scaler.fit(rad_train_2d)
        rad_train_scaled = rad_scaler.transform(rad_train_2d)
        rad_val_scaled = rad_scaler.transform(rad_val.view(rad_val.size(0), -1).numpy())
        ...
        rad_train_tensor = torch.from_numpy(rad_train_scaled).float().view(rad_train.shape).to(device)
        rad_val_tensor = torch.from_numpy(rad_val_scaled).float().view(rad_val.shape).to(device)
        labels_train = labels_train.to(device)
        labels_val = labels_val.to(device)

        # Create TensorDatasets and DataLoaders with train/val data
        train_dataset = TensorDataset(hist_train_tensor, rad_train_tensor, labels_train)
        val_dataset = TensorDataset(hist_val_tensor, rad_val_tensor, labels_val)
        ...
        rad_scaler.fit(rad_train_2d)
        rad_train_scaled = rad_scaler.transform(rad_train_2d)
        rad_test_scaled = rad_scaler.transform(rad_test.view(rad_test.size(0), -1).numpy())

        # Reshape back to original dimensions and convert to tensors 
        hist_train_tensor = torch.from_numpy(hist_train_scaled).float().view(hist_train.shape).to(device)
        hist_test_tensor = torch.from_numpy(hist_test_scaled).float().view(hist_test.shape).to(device)
        rad_train_tensor = torch.from_numpy(rad_train_scaled).float().view(rad_train.shape).to(device)
        rad_test_tensor = torch.from_numpy(rad_test_scaled).float().view(rad_test.shape).to(device)
        ...
        train_dataset = TensorDataset(hist_train_tensor, rad_train_tensor, labels_train)
        test_dataset = TensorDataset(hist_test_tensor, rad_test_tensor, labels_test)
        train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
        ...
        for epoch in range(num_epochs):
            ...
            for hist_embedding, rad_embedding, labels in train_loader:
                hist_embedding, rad_embedding, labels = hist_embedding.to(device), rad_embedding.to(device), labels.to(device)
                
                optimizer_attention.zero_grad()
                optimizer_eval.zero_grad()
                fused_embedding = attention_model(hist_embedding, rad_embedding)
                durations = labels[:, 0]
                events = labels[:, 1]
                ...
                log_hazards = net(fused_embedding)
        ...
        with torch.no_grad():
            for hist_embedding, rad_embedding, labels in test_loader:
                hist_embedding, rad_embedding, labels = hist_embedding.to(device), rad_embedding.to(device), labels.to(device)
                fused_embedding = attention_model(hist_embedding, rad_embedding)
                fused_embedding, labels = fused_embedding.to(device), labels.to(device)
                log_hazards = net(fused_embedding)
...
def train_with_divergence_stopping(attention_model, 
                                   evaluation_model, 
                                   train_loader, 
                                   val_loader, 
                                   criterion, 
    ...
    Args:
        attention_model: Neural network model for fusing histogram and radiology embeddings
        evaluation_model: Neural network model for survival analysis prediction
        train_loader (DataLoader): Training data loader containing (hist_embedding, rad_embedding, labels)
        ...
        optimizer_evaluation: Optimizer for the evaluation model parameters
        optimizer_attention: Optimizer for the attention model parameters
    ...
    Example:
        ...
        ...     evaluation_model=survival_net,
        ...     train_loader=train_dl,
        ...     val_loader=val_dl,
        ...     criterion=cox_loss,
        ...     optimizer_evaluation=optim_eval,
        ...     optimizer_attention=optim_att,
    ...
    for epoch in range(max_epochs):
        ...
        for hist_embedding, rad_embedding, labels in train_loader:
            hist_embedding, rad_embedding, labels = hist_embedding.to(device), rad_embedding.to(device), labels.to(device)
            optimizer_attention.zero_grad()
            optimizer_evaluation.zero_grad()
            fused_embedding = attention_model(hist_embedding, rad_embedding)
            fused_embedding, labels = fused_embedding.to(device), labels.to(device)
            ...
            log_hazards = evaluation_model(fused_embedding)
            loss = criterion(log_hazards, durations, events)
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(evaluation_model.parameters(), max_norm=1.0)
            optimizer_attention.step()
            optimizer_evaluation.step()
        ...
        all_durations, all_events, all_scores = [], [], []
        with torch.no_grad():
            for hist_embedding, rad_embedding, labels in val_loader:
                hist_embedding, rad_embedding, labels = hist_embedding.to(device), rad_embedding.to(device), labels.to(device)
                fused_embedding = attention_model(hist_embedding, rad_embedding)
                fused_embedding, labels = fused_embedding.to(device), labels.to(device)
