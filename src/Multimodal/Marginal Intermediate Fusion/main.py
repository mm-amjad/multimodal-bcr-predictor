...
def main():
    ...
    for npy_file in npy_files:
        ...
        input_dim = radiology_embedding.shape[0]
        output_dim = histology_embedding_dim

        # Linearly project radiology_embedding to same dimension as histology_embedding
        np.random.seed(42)
        projection_matrix = np.random.randn(input_dim, output_dim) / np.sqrt(input_dim)
        radiology_embedding = radiology_embedding @ projection_matrix
        patient_id = int(npy_file.stem[:4])
    ...
    concatenated_vector_dim = 0
    labels = []
    for patient_id, data in patient_data.items():
        histology_embedding = data["histology_embedding"]
        radiology_embedding = data["radiology_embedding"]
        concatenated_vector = np.concatenate([histology_embedding, radiology_embedding])
        concatenated_embeddings(concatenated_vector)
        labels.append(data["BCR"])

        if concatenated_vector_dim == 0:
            concatenated_vector_dim = concatenated_vector.shape[0]

    concatenated_embeddings_tensor = torch.tensor(concatenated_embeddings, dtype=torch.float32)
    labels_tensor = torch.tensor(labels, dtype=torch.long)

    # Create TensorDataset with concatenated embeddings and labels
    patient_dataset = TensorDataset(concatenated_embeddings_tensor, labels_tensor)
    ...
    for i, (batch_size, layer_sizes, dropout) in enumerate(param_grid):
        ...
        for i in range(num_repeats):
            ...
            results_for_run = run_cross_validation(
                ...
                embedding_dim=concatenated_vector_dim
            )
